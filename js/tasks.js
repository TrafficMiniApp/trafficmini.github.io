    ...
    clientTasks.forEach(task => {
        ...
        taskEl.innerHTML = `
            <div class="task-stats">
                ...
                ${isCancelled ? `<div style="color: #f77307ff; margin-bottom: 10px;">${getTranslation('taskCancelled', 'Cancelled')}</div>` : ''}
                ...
                ${!isCancelled ? `<button class="mini-btn cancel-task-btn" style="width: 100%; margin-top: 15px; background: #f77307ff;" onclick="handleCancelTask(${task.id})">${getTranslation('cancelTask', 'Cancel Task')}</button>` : ''}
            </div>
        `;
